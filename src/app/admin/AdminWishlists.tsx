...
import { Heart, RefreshCw, Mail } from 'lucide-react';
    ...
    if (wishlists.length === 0) {
        return (
            <div className="text-center py-24 border border-dashed border-gray-300 rounded">
                ...
                <p className="text-gray-500 font-medium">No items wishlisted yet.</p>
            </div>
        );
    }
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-4">
                <p className="text-sm text-gray-500 font-medium">{wishlists.length} wishlisted items</p>
                <button onClick={fetchWishlists} className="text-xs font-bold uppercase tracking-widest border border-gray-300 px-3 py-1.5 hover:bg-gray-50 transition flex items-center gap-1.5">
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                            ...
                            <th className="text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400">Wishlisted On</th>
                        </tr>
                    </thead>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                                                <span className="bg-gray-100 px-1.5 py-0.5 rounded text-[10px]">{entry.products?.category || 'No Category'}</span>
                                                <span className="font-mono text-gray-400">₹{entry.products?.price}</span>
                                            </p>
                                        </div>
                                    </div>
                                </td>
